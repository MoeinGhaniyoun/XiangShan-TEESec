...
class PTWImp(outer: PTW)(implicit p: Parameters) extends PtwModule(outer) with HasCSRConst with HasPerfEvents {
  ...
  ////////////////////////////////
  printf("PTWCacheInfo: ReqValid: %d, ReqVPN: %x, RefilValid: %d RefillPTEs: %x, CacheHit: %d\n", cache.io.req.valid, arb1.io.out.bits.vpn, cache.io.refill.valid, cache.io.refill.bits.ptes, cache.io.resp.bits.hit);

  printf("mem_resp_done: %d, flush: %d, flush_latch(mem.d.bits.source): %d, mem.d.bits.source: %x, mem.d.valid: %d\n", mem_resp_done, sfence.valid, flush_latch(mem.d.bits.source), mem.d.bits.source, mem.d.valid);
